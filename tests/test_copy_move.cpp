...
TEST(CopyMove, FuncEvalMany) {
    ...
    auto fe1 = fit<5>(f1, -1.0, 1.0);
    auto fe2 = fit<5>(f2, -1.0, 1.0);
    auto base = pack(fe1, fe2);
    ...
    auto temp2 = pack(fe1, fe2);
    ref = temp2(-0.6);
    FEM move_assign = pack(fe1, fe2);
    move_assign = std::move(temp2);
...
TEST(CopyMove, FuncEvalND) {
    ...
    FEN copy_assign = fit<3>(f, low, high);
    copy_assign = base;
    val = copy_assign(x);
    EXPECT_NEAR(ref[0], val[0], 1e-12);
    EXPECT_NEAR(ref[1], val[1], 1e-12);

    auto temp = fit<3>(f, low, high);
    Out ref2 = temp(x);
    ...
    auto temp2 = fit<3>(f, low, high);
    Out ref3 = temp2(x);
    FEN move_assign = fit<3>(f, low, high);
    move_assign = std::move(temp2);
    val = move_assign(x);
